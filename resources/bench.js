...
Promise.all([
  readFile(path.resolve(__dirname, '../dist/nuclear.js'), { encoding: 'utf8' }),
  exec('git show master:dist/nuclear.js', {maxBuffer: 1024 * 500})
]).then(function (args) {
  var newSrc = args[0];
  var oldSrc = args[1];
  ...
  return readdir(perfDir).then(function (filepaths) {
    ...
    modules.forEach(function (Nuclear, version) {
